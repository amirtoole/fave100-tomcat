 ...
 * http://svn.apache.org/viewvc/lucene/solr/trunk/src/java/org/apache/solr/analysis/PatternReplaceCharFilter.java
 */
...
public class PatternReplaceCharFilter extends BaseCharFilter
{
	...
	public PatternReplaceCharFilter(final String pattern, final String replacement, final MappingCharFilter in) {
		this(Pattern.compile(pattern), replacement, DEFAULT_MAX_BLOCK_CHARS, null, in);
	}

	public PatternReplaceCharFilter(final Pattern pattern, final String replacement, final MappingCharFilter in) {
		this(pattern, replacement, DEFAULT_MAX_BLOCK_CHARS, null, in);
	}

	public PatternReplaceCharFilter(final Pattern pattern, final String replacement,
									final int maxBlockChars, final MappingCharFilter in) {
		this(pattern, replacement, maxBlockChars, null, in);
	}

	public PatternReplaceCharFilter(final Pattern pattern, final String replacement,
									final String blockDelimiters, final MappingCharFilter in) {
		this(pattern, replacement, DEFAULT_MAX_BLOCK_CHARS, blockDelimiters, in);
	}

	public PatternReplaceCharFilter(final Pattern pattern, final String replacement,
									final int maxBlockChars, final String blockDelimiters, final MappingCharFilter in) {
		super(in);
	...
	private void pushLastChar(final int c) {
		if (buffer == null) {
	...
	String getReplaceBlock(final String block) {
		final char[] blockChars = block.toCharArray();
		return getReplaceBlock(blockChars, 0, blockChars.length);
	}

	String getReplaceBlock(final char block[], final int offset, final int length) {
		final StringBuffer replaceBlock = new StringBuffer();
		final String sourceBlock = new String(block, offset, length);
		final Matcher m = pattern.matcher(sourceBlock);
		...
		while (m.find()) {
			...
			final int diff = replaceBlock.length() - lastMatchOffset - lastDiff - (m.end(0) - lastMatchOffset);
			if (diff != 0) {
