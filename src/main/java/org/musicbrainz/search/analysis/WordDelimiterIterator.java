...
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.ALPHA;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.DIGIT;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.LOWER;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.SUBWORD_DELIM;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.UPPER;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.isAlpha;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.isDigit;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.isSubwordDelim;
import static org.musicbrainz.search.analysis.MusicbrainzWordDelimiterFilter.isUpper;

/**
 * A BreakIterator-like API for iterating over subwords in text, according to MusicbrainzWordDelimiterFilter rules.
 * 
 * @lucene.internal
 */
public final class WordDelimiterIterator {
	...
	int next() {
		...
		for (end = current + 1; end < endBounds; end++) {
			final int type = charType(text[end]);
	...
	int type() {
		...
		final int type = charType(text[current]);
		switch (type) {
	...
	private boolean endsWithPossessive(final int pos) {
		return (stemEnglishPossessive &&
				...
				isAlpha(charType(text[pos - 3])) && (pos == endBounds || isSubwordDelim(charType(text[pos]))));
	}
