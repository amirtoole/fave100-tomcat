 ...
 * 
 * One use for MusicbrainzWordDelimiterFilter is to help match words with different subword delimiters.
...
public class MusicbrainzWordDelimiterFilter extends TokenFilter {
	...
	protected final TypeAttribute typeAttribute = addAttribute(TypeAttribute.class);
	 ...
	 */
	public MusicbrainzWordDelimiterFilter(final TokenStream in,
	 ...
	 * 
	 * @deprecated Use {@link #MusicbrainzWordDelimiterFilter(TokenStream, byte[], int, int, int, int, int, int, int, int, int, CharArraySet)} instead.
	 */
	@Deprecated
	public MusicbrainzWordDelimiterFilter(final TokenStream in,
											final byte[] charTypeTable,
	...
	/**
	 * @deprecated Use {@link #MusicbrainzWordDelimiterFilter(TokenStream, int, int, int, int, int, int, int, int, int, CharArraySet)} instead.
	 */
	@Deprecated
	public MusicbrainzWordDelimiterFilter(final TokenStream in,
											final int generateWordParts,
	...
	public final boolean incrementToken() throws IOException {
		while (true) {
			if (!hasSavedState) {
				...
				//Use Word Delimter just on these tokens
				if (typeAttribute.type() != ALPHANUMANDPUNCTUATION) {
					return true;
				}

				final int termLength = termAttribute.length();
				final char[] termBuffer = termAttribute.buffer();
	...
	private void generatePart(final boolean isSingleWord) {
		...
		final int startOffSet = (isSingleWord || !hasIllegalOffsets) ? savedStartOffset + iterator.current : savedStartOffset;
